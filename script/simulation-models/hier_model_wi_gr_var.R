# Hierachical Gaussian example: misspecified cross observations by double the uncertainty
...
simulate_data_mis <- function(num_of_groups, num_of_individuals){
  true_thetas <- rnorm(num_of_groups)
  X <- matrix(rnorm(num_of_groups * num_of_individuals, true_thetas, sd = sqrt(8)),  num_of_groups, num_of_individuals)
  return(list("X_obs" = X, "para" = true_thetas))
}
